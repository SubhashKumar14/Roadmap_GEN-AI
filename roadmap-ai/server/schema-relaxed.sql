...
-- Create helper functions for likes and forks
CREATE OR REPLACE FUNCTION increment_likes(roadmap_id UUID)
RETURNS void AS $$
BEGIN
  UPDATE roadmaps SET likes_count = likes_count + 1 WHERE id = roadmap_id;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION decrement_likes(roadmap_id UUID)
RETURNS void AS $$
BEGIN
  UPDATE roadmaps SET likes_count = GREATEST(likes_count - 1, 0) WHERE id = roadmap_id;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION increment_forks(roadmap_id UUID)
RETURNS void AS $$
BEGIN
  UPDATE roadmaps SET forks_count = forks_count + 1 WHERE id = roadmap_id;
END;
$$ LANGUAGE plpgsql;
...
ALTER TABLE roadmap_likes DISABLE ROW LEVEL SECURITY;
